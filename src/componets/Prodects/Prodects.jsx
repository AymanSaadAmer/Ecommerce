    ...
    <>
      <div className="row w-[92%] m-auto">
         <div className='w-[100%] mb-0'>
              <h2 className='!text-green-800 font-medium text-xl !mt-5'>Shop current products :- </h2>
    </div>
              {data.map((prodect)=> 
                <div key={prodect.id} className='w-1/2 md:w-1/4 lg:w-1/6 '>
                 
                <div className="prodect py-3 px-3">
                    <Link className="!no-underline text-inherit " to={`/ProdectDetails/${prodect.id}/${prodect.category.name}`}>
            <Slider {...settings}>
              {prodect?.images.map((src)=><img className='w-full' src={src} alt={prodect?.tittle} />)}
            </Slider>
